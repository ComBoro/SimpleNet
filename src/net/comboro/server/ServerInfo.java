...
public class ServerInfo {
     ...
     * prevented users
     *
     * @param string the String version of the {@link InetAddress} that is going to
     *               be banned
     */
     ...
     *
     * @param writer the initialised {@link PrintWriter} with the open
     *               configuration file
     */
    ...
    private void loadBanList() {
        ...
        readBanList(Objects.requireNonNull(banListReader));
    }
    ...
    private void loadLogger() {
        try {
            ...
            Server.error("Failed to load the logger. Exception: "
                    + e.getMessage());
        }
    }
    ...
    private void loadServerConfig() {
        ...
        // Use Reader && Writer
        boolean result = readConfig(Objects.requireNonNull(reader));
        if (!result) {
            Server.error("Invalid config");
            createDefault(Objects.requireNonNull(writer));
            writer.close();
            reader = getReader(serverConfig);
            readConfig(Objects.requireNonNull(reader));
            closeRedaer(reader);
            return;
        }
     ...
     *
     * @param banListReader the initialised {@link BufferedReader} containing the ban list
     *                      file
     */
     ...
     *
     * @param reader the initialised {@link BufferedReader} with the open
     *               configuration file
     ...
     *
     * @param string the String version of the {@link InetAddress} that will be
    ...
    private void updateBanListFile() {
        try {
            ...
            while (it.hasNext())
                Objects.requireNonNull(writer).println(it.next());
            set.clear();
